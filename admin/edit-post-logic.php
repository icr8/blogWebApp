...
if(isset($_POST['submit'])){
    ...
    //check an validate input values
    if(!$title){
    ...
    elseif(!$body){
        ...
        if($thumbnail['name']){
            $previous_thumbnail_path = '..img/images/' . $previous_thumbnail_name;
            ...
            //Rename image
            $time = time(); //make image name unique using current timestamp
            $thumbnail_name = $time . $thumbnail['name'];
            $thumbnail_tmp_name = $thumbnail['tmp_name'];
            $thumbnail_destination_path = '../img/images/' . $thumbnail_name;
        ...
        if(in_array($extention, $allowed_files)){
            ...
            if($thumbnail['size'] < 2000000){
                ...
                $_SESSION['edit-post'] = "Couldn't update post. File size too big. It should be less than 2mb";
            }
        } else{
